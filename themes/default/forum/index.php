...
<div class="max-w-5xl mx-auto px-4 sm:px-6 py-10">
  ...
  <div class="flex items-center justify-between mb-8">
    <div>
      ...
      <p class="text-slate-400 mt-1">Diskuter alt med fællesskabet</p>
    </div>
  </div>
  ...
  <div class="glass rounded-2xl p-12 text-center">
    ...
    <p class="text-slate-400 font-medium mb-1">Ingen kategorier endnu</p>
    <?php if(auth()->isAdmin()): ?>
    <p class="text-sm text-slate-600">Opret kategorier i <a href="<?= url('admin/forum') ?>" class="text-brand hover:underline">Admin → Forum</a>.</p>
    <?php endif; ?>
  </div>
  <?php else: ?>
    ...
    <a href="<?= url('forum/category/' . urlencode($cat['slug'])) ?>"
      ...
      <div class="hidden sm:flex items-center gap-6 text-center flex-shrink-0">
        <div>
          ...
          <p class="text-xs text-slate-500">Tråde</p>
        </div>
        <div>
          <p class="text-lg font-bold text-white"><?= number_format((int)$cat['post_count']) ?></p>
          <p class="text-xs text-slate-500">Svar</p>
        </div>
      </div>
