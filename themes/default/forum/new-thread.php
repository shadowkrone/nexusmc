<?php $pageTitle = t('forum.new_thread'); ?>
<div class="max-w-3xl mx-auto px-4 sm:px-6 py-10">
  <nav class="flex items-center gap-2 text-sm text-slate-500 mb-6">
    ...
    <span>›</span>
    <span class="text-slate-300"><?= t('forum.new_thread_breadcrumb') ?></span>
  </nav>

  <h1 class="text-2xl font-bold text-white mb-7"><?= t('forum.create_new_thread') ?></h1>
  ...
  <div class="glass rounded-2xl p-6">
    <form method="POST" action="<?= url('forum/thread/create') ?>" class="space-y-5">
      ...
      <div>
        <label class="block text-sm font-medium text-slate-300 mb-1.5"><?= t('forum.title_required') ?></label>
        ...
        <p class="text-xs text-slate-600 mt-1"><?= t('forum.title_hint') ?></p>
      </div>

      <div>
        <label class="block text-sm font-medium text-slate-300 mb-1.5"><?= t('forum.body_label') ?></label>
        <textarea name="body" rows="10" required
                  ...
                  placeholder="<?= t('forum.body_ph') ?>"></textarea>
      </div>

      <div class="flex items-center justify-between pt-2">
        <a href="<?= url('forum/category/' . urlencode($category['slug'])) ?>"
           class="text-sm text-slate-400 hover:text-white transition-colors"><?= t('forum.cancel') ?></a>
