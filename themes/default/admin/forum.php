<div class="max-w-4xl">
  <h1 class="text-2xl font-bold text-white mb-7">Forum kategorier</h1>
  ...
  <div class="grid lg:grid-cols-2 gap-6">
    ...
    <div class="glass rounded-2xl p-6">
      <h2 class="font-semibold text-white mb-5">Opret kategori</h2>
      <form method="POST" action="<?= url('admin/forum/category') ?>" class="space-y-4">
        <?= csrf_field() ?>
        <div>
          <label class="block text-sm font-medium text-slate-300 mb-1.5">Navn</label>
        ...
        <div>
          <label class="block text-sm font-medium text-slate-300 mb-1.5">Beskrivelse</label>
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          <div>
            <label class="block text-sm font-medium text-slate-300 mb-1.5">Farve</label>
    ...
    <div class="glass rounded-2xl overflow-hidden">
      <div class="px-5 py-4 border-b border-slate-800">
        <h2 class="font-semibold text-white">Eksisterende kategorier</h2>
      </div>
      <?php if(empty($categories)): ?>
      <div class="p-8 text-center text-slate-500">Ingen kategorier oprettet endnu.</div>
      <?php else: ?>
      <div class="divide-y divide-slate-800/50">
        ...
        <div class="flex items-center gap-3 px-5 py-3">
          ...
          <form method="POST" action="<?= url('admin/forum/category/' . $cat['id'] . '/delete') ?>"
                onsubmit="return confirm('Slet denne kategori og ALLE dens tråde?')">
            <?= csrf_field() ?>
